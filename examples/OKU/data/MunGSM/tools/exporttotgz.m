function tgzfilename = exporttotgz(funcname,abspath,tgzfilename)
%% EXPORTTOtgz - Creates a tgz file containing dependencies of a function
%
% tgzfilename = exporttotgz(funcname);
% tgzfilename = exporttotgz(funcname,abspath);
% tgzfilename = exporttotgz(funcname,abspath,tgzfilename);
%
% Finds and tgzs all M-files which are required by the specified function(s) 
...
%               1 = use absolute paths, write the tgzfile to the local directory
%               0 = use relative paths, writhe the tgzfile to the commom rootpath
% tgzfilename   the tgzfile to export the functions to
%
% abspath is useful when you are using networked drives where a common
% rootpath may not exist or be useful
%
% If no tgzfilename is specified, the tgz file is created in the current
% working directory with name X.tgz where X is the first function name in
% the supplied list
%
% The file names in the tgz file are relative to the
...
if nargin<3
    tgzfilename = fullfile(pwd,[ funcname{1} '.tgz' ]);
end
