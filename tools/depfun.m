...
%    list. Script M-files can be included but the dependency analysis is
%    wildly conservative.
%
...
%    $Revision: 1.52.4.19 $ $Date: 2008/03/17 22:14:30 $
        ...
        while i <= nin
            if isa(varargin{i},'char')
                switch varargin{i}
                  case '-toponly'
                    toponly = true;
                    ndf_options = { ndf_options{:}, '-toponly' };
                  case '-nographics'
...
%---------------------------------------------------------------------------
    function  prepare_trace
        ...
        if nx ~= 0
            ...
            for k=1:nx
                ...
                if ~isempty(cbnames)
                    yyy = create_callback_M_file(cbnames,cbstrings,k);
                    tmp_files{k} = yyy;
                end
            end
       ...
       % builtins: Determine the names
        ...
        if ~isempty(builtins)
          ...
          for i=1:length(builtins)
            [unusedTempVar,names{i}] = fileparts(builtins{i});
          end
    ...
    %---------------------------------------------------------------------------
    function [file] = create_callback_M_file(cbnames,cbstrings,k)
                  ...
                  if isempty(tmp_files)
                      ...
                      for iTl=1:length(trace_list)
                          ...
                          if any(find(tmp_files_ix == iTl))
                              ntmp = ntmp + 1;
                              format = '          %s\n';
                              fprintf(fid,format,['-> Generated M-file' ...
        ...
        if any(strcmp(hg_cbnames,lstructnames{j}))
            ...
            elseif ~isempty(fi)
                warning('MATLAB:DEPFUN:NotAStringForCallback','%s %s %s', ...
                       'Callback for name ''', strtrim(lstructnames{j}), ''' is not a string');
            end
        elseif isstruct(fi)
...
function [trace_file,off] = next_arg_file(arg_file)
    ...
    %
    [unusedTempVar,f,e] = fileparts(arg_file);
    ...
    error('MATLAB:DEPFUN:FileDoesNotExist','The file ''%s'' %s', ...
            arg_file, 'does not exist');
end
