%% mstruct
% mstruct Manage Variables in a scalar structure.
function varargout = mstruct(varargin)

%% Description
%    mstruct has dual functionality in packing & unpacking variables into structures and
...
%      S = mstruct
%      S = mstruct(x,y,z,...)
%      S = mstruct(fieldNames)
%      S = mstruct(A,B,C,..., fieldNames)
%      S = mstruct(x,..., nameOfStruct2Update, fieldNames)
%      mstruct
%      mstruct(x,y,z,...)
%      mstruct(fieldNames)
%      mstruct(A,B,C,..., fieldNames)
%      mstruct(x,..., nameOfStruct2Update, fieldNames)
%
%    Unpack
%      mstruct(S)
%      [a,b,c,...] = mstruct(S)
%      mstruct(S,fieldNames)
%      [a,b,c,...] = mstruct(S,fieldNames)
%
%% Inputs & Outputs
%    Pack - inputs
...
%          Smstruct would be created in the caller workspace.
%
%    Unpack - inputs
...
%       S = mstruct(fieldNames1) 
%      % Example 3.
%      % same as #1. but arguments are passed explicitly
%       S = mstruct(zeros(3), 'Testing123', cell(2,3), fieldNames1) 
%      % Example 4.
...
%       S = mstruct(x2, nameOfStruct2Update, fieldNames3)
%
%      % Example 6.
...
%      % structure would be returned with the default name 'Smstruct'. Be cautious as
...
%       mstruct(fieldNames1)
%      % Example 9.
%       mstruct(zeros(3), 'Testing123', cell(2,3), fieldNames1)
%      % Example 10.
%       mstruct(x,y,z, fieldNames2)
%      % Example 11.
%       S.oldField = 'field to be saved for future use'
%       mstruct(x2, nameOfStruct2Update, fieldNames3)
%      % Example 12.
%       mstruct
%
%  % Unpack
...
%    1. run attached mstructDemo1.m file for on screen presentation of examples.
%    2. run attached mstructDemo2.m file and read comments in file for a suggestion of
%       how to use mstruct in managing input to other functions with improved usability.
%
%% Revision history
   ...
   if ~gotFieldNames
      ...
      if isequal(nInputs, 0)
         ...
         if gotCellArrayOfStrings
            ...
            warnMsg = ['mstruct - ''%s packed in the structure.'...
               '\nTo avoid this warning do not put ''%s'' as last mstruct input.'...
               '\nIf you want to pack (or unpack) using ''%s'' as designated names'...
               ' of the'...
               '\nstructure''s fields, add a cell with the string ''fieldNames'' to'...
               ' ''%s''.'];
            fprintf('\n')
            warning('MATLAB:mstruct:cellArrayOfStringNotFieldNames',warnMsg,msgStr,...
                     name,name,name)
         end
      end
