...
function [g_cpi, cpi]= sg_cpiT(g_T, T)
   % calculate ice isobaric heat capacity [J/(kg*K]
   % regression on published data
   [n1, n2]= size(T); if n1== 1, g_tmp_cpiT_00003= g_T' ;
      tmp_cpiT_00003= T' ; % Update detected: T= some_expression(T,...)
      g_T= g_tmp_cpiT_00003;
      T= tmp_cpiT_00003;
   end
   % Identifier 'size' is ignored during differentiation.
   tmp_size_00001= size(T);
   g_tmp_size_00001= zeros(size(tmp_size_00001));
   g_cpi= 0.00001* zeros(tmp_size_00001);
   cpi= 0.00001* ones(tmp_size_00001); 
   clear tmp_size_00001 g_tmp_size_00001 ;
   tmp_cpiT_00001= T< 0;
   g_tmp_cpiT_00001= zeros(size(tmp_cpiT_00001));
   g_cpi(T< 0)= 7.7* g_T(tmp_cpiT_00001);
   cpi(T< 0)= 2110+ 7.7* T(tmp_cpiT_00001); 
   clear tmp_cpiT_00001 g_tmp_cpiT_00001 ;
