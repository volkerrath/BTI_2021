...
if res.dims==1 
   if nargin>2 
      for i= 1: res.ndd
         res.deriv{i}= feval(func, g.deriv{i}, varargin{:});
      end;
   else
      for i= 1: res.ndd
         res.deriv{i}= feval(func, g.deriv{i});
      end;
   end
else
   if nargin>2
      for i= 1: res.ndd(1)
         for j=1: res.ndd(2)
            res.deriv{i,j}= feval(func, g.deriv{i,j}, varargin{:});
         end
      end
   else
      for i= 1: res.ndd(1)
         for j=1: res.ndd(2)
            res.deriv{i,j}= feval(func, g.deriv{i,j});
         end
      end
   end
end
