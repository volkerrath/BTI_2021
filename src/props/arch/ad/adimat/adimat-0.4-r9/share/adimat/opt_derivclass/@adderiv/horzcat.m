...
elseif nargin==2 % Faster, because no reallocation occurs.
   ...
   if ~(isa(varargin{1}, 'adderiv') && isa(varargin{2}, 'adderiv'))
      error('Both arguments of a horizontal matrix concatenation have to be adderivs.');
   end;
   ...
   if ~isa(varargin{c}, 'adderiv')
      error(sprintf('All arguments of a horizontal matrix concatenation have to be adderivs. %d is a %s', c, isa(varargin{c})));
   end;
