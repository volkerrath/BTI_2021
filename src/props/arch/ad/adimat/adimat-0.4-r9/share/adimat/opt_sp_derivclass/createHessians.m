...
if isempty(g_dirs)
   g_dirs= get(adderivsp([], [], 'empty'), 'NumberOfDirectionalDerivatives');
   if g_dirs(1)==1
...
for i=1: nargout
   ...
   if isstruct(varargin{i})
      ...
      if length(sz)>2
         varargout{i}= adderivsp(g_dirs, sz, 'zeros');
      else
         varargout{i}= adderivsp(g_dirs, sz, 'sparse');
      end
   end
end
