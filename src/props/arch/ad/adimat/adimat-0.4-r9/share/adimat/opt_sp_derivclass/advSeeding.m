function [g_object, lddu]=advSeeding(g_object, coloring, varargin)
  ...
  %
  % G_R= advSeeding(G_X, COLORING)
  %      Advanced seeding needs a COLORING pattern of the derivative's data
  %      which is usually known through structure of the problem or by 
  %      evaluating the Jacobian once and then applying color() on the 
  %      Jacobian which gives the coloring of the columns in the Jacobian.
  %      G_R{i}(j) is G_X{i}(j) with ones inserted at the positions, where
  %      COLORING(j)= i. That is, all j-th entries with the same color i
  %      are inserted in the same directional derivative i in G_R.
  %
  % [G_R, LDDU]= advSeeding(...)
  %      Returns the greatest directional derivative i used by the 
  %      coloring. If a startdd (starting directional derivative) N is
  %      given, then LDDU=N+ max(COLORING), else LDDU= max(COLORING).
  %
  % ...= advSeeding(G_X, COLORING, OPTIONS)
  %      For OPTIONS one or more of the following may be given. If options
  %      are specified multiple times, the value of the last occurrence
  %      is used.
  %
  %      Valid options are:
...
% Copyright 2005-2006 Andre Vehreschild, Institute for Scientific Computing   
   ...
   if nargin>2
      ...
      for c=1: 2: (nargin-2)
         switch varargin{c}
            ...
            otherwise
               warning('Unknown option "%s". Ignored!', varargin{c});
         end
      end
   end
