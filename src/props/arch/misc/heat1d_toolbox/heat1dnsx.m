function [T,z,k_eff,ipor] ...
    = heat1dns(kl,kAl,kBl,hl,porl,qb,gt,index,dz,maxiter,tol,freeze,out)
%ADiMat AD_DVARS= T
%ADiMat AD_IVARS= kl,hl,qb,gt
%ADiMat BMFUNC $$=spdiags($1, $#) DIFFTO call(@spdiags, $@1, $#)    
%ADiMat AD_OUTPUT_DIR="ad_out_s/"
% HEAT1DNS solves nonlinear stationary heat equation
% for general 1D grids (permafrost included).
%
% T = heat1dns(k,kA,kB,h,por,qb,gt,index,dz,maxiter,tol,out) calculates
...
for iter=1:maxiter
    ...
    if iter>1,
        if T-Titer~=0,
            ...
            if strcmpi(lower(out),'yes') ~=0,
                disp([' maximal deviation from last iteration is ',...
                    num2str(checktol), ' K']);end
            if checktol <= tol, break; end
        else
            disp([' maximal deviation from last iteration is zero']);end
        break;
    end
end
Titer=T;
