function [T,z,t,N,k_eff,rc_eff,ipor,lheat,rci]=...
    heat1dnt(kl,kAl,kBl,hl,rhoml,cpml,rcml,porl,Qb,...
    zones,dz,dt,T0,Ts,tip,theta,maxiter,tol,freeze,out)
%ADiMat AD_DVARS= T
%ADiMat AD_IVARS= kl,hl,rcml,porl,Qb,Ts
%ADiMat BMFUNC $$=spdiags($1, $#) DIFFTO call(@spdiags, $@1, $#)    
%ADiMat AD_OUTPUT_DIR="ad_out_t/"
% HEAT1DNTP  solves nonlinear time-dependent heat equation
% for general 1-D grid with permafrost.
%
% T = heat1dnt(par,nu,zones,dz,dt,T0,Ts,methods) calculates
...
% porl			= porosity
% Qb			= basal heat flow
% zones (1:nc)     	= pointer to assign parameters to cells
...
[n1 n2]=size(zones); if n1==1, zones=zones';end
[n1 n2]=size(dz);    if n1==1, dz=dz';end
[n1 n2]=size(dt);    if n1==1, dt=dt';end

nt=length(dt)+1;
nc=length(zones);nz=nc+1;

k=kl(zones(1:  nc));          % thermal conductivity
[n1 n2]=size(k);    if n1==1, k=k';end
kA =    kAl(zones(1:  nc));   % thermal conductivity coefficient A
[n1 n2]=size(kA);   if n1==1, kA=kA';end
kB =    kBl(zones(1:  nc));   % thermal conductivity coefficient B
[n1 n2]=size(kB);   if n1==1, kB=kB';end
h  =     hl(zones(1:  nc));   % heat production
[n1 n2]=size(h);    if n1==1, h=h';end
rhom  = rhoml(zones(1:  nc));   % density
[n1 n2]=size(rhom); if n1==1, rhom=rhom';end
cpm0   =  cpml(zones(1:  nc));   % heat capacity
[n1 n2]=size(cpm0);  if n1==1, cpm0=cpm0';end
rcm0   =  rcml(zones(1:  nc));   % heat capacity
[n1 n2]=size(rcm0);  if n1==1, rcm0=rcm0';end
por   =  porl(zones(1:  nc));   % pcorosity
[n1 n2]=size(por);  if n1==1, por=por';end
%
one=ones(size(zones));
zero=zeros(size(zones));
%INITIAL VALUES
...
Tlast=T0;Titer=T0;Tlast(1)=Ts(tip(1));Titer(1)=Ts(tip(1));
...
for i = 1:nt-1
    ...
    for  iter =1:maxiter
        ...
        cpm0=rcm0./rhom; cpm=cpmT(cpm0,Tc); km=kmT(k,Tc,kA,kB);
