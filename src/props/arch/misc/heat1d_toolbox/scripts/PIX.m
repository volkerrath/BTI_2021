...
tau0 = 0.2;		% weight for identity matrix  
tau1 = logspace(-1,1,32);               % weight for Gradient
maxiter   = 16;         % number of inverse iterations
...
L1(np,np)=0;LS=L1'+tau_ratio*L0;
[K,R]=qr(LS');[n1,n2] = size(LS);R=R(1:n2,:);LI=inv(LS);
...
%         end
    ...
    Wt= sqrt(tau1)*speye(size(LS));St = S/LS;  
    
    disp([' ...calculate parameter increment ']);
%    S_Aug=[St;Wt];
%    r_Aug=[r_tot;-Wt*(m'-m_apr'+gt)];
%    [delta_m] = cglsACB(S_Aug,r_Aug,reorth,kc,tolcgls,stoprule,0);    
    [mall,active] = cglssh(S,R0,k,lambda,tol);
