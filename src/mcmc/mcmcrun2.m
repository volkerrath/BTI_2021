function [results,parchain,reschain,calchain,s2chain,sschain,hchain]=mcmcrun2(model,data,params,options,res)
...
calchainfile= getpar(options,'calchainfile',sprintf('calchain_%05d.mat',fnum));
...
if ~isempty(savedir)
  ...
  calchainfile = [savedir,calchainfile];
end
...
for isimu=2:nsimu % simulation loop
  ...
  if any(newpar<low(parind)) || any(newpar>upp(parind))
    ...
    % calculate ss
    ss2 = ss;             % old ss
    [ss1,newres,newcal] = sseval(ssfun,ssstyle,newpar,parind,value,local,data,modelfun);
  ...
  if dodram == 1 && accept == 0 % & outbound == 0
    ...
    while accept == 0 & itry < Ntry
      ...
      outbound = 0;
      [z.ss z.res,z.cal] = sseval(ssfun,ssstyle,z.p,parind,value,local,data,modelfun);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ss res,cal] = sseval(ssfun,ssstyle,theta,parind,value,local,data,modelfun)
% evaluate the "sum-of-squares" function

value(parind) = theta;
if ssstyle == 1

  [ss res, cal] = feval(ssfun,value(:)',data);
elseif ssstyle == 4

  [ss res, cal] = mcmcssfun(value(:)',data,local,modelfun);

else

  [ss res,cal] = feval(ssfun,value(:)',data,local);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
